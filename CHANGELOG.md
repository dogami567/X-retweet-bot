...
- 关键词采集：支持从 X 搜索页按关键词采集「最新/热门」推文 URL，采集后自动去重入队，可一键“采集并开始”。
- 采集稳定性：搜索页出现 “Something went wrong” 会自动点 Retry；DOM 抓不到链接时启用 HTML 兜底提取 `/status/<id>`，降低 0 结果概率。
- 测试：新增 Playwright 全量测试 + 真实验收用例（真实访问 x.com/search），确保“采集 → 入队 → UI 可见”闭环跑通。
- 发布包：Windows EXE ZIP 中会附带小白图文教程（docx + 截图资源），方便分发。
