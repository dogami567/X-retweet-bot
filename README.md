...
3. 设置「每号本次关注上限」（例如 10）
4. 点击「开始」
5.（可选）设置「等待关注按钮（秒）」：默认 18 秒；如果页面加载慢/风控导致按钮久不出现，可调大（如 180 秒）
6.（可选）设置「并行账号数」：默认 1（串行）；并发会同时打开多个浏览器窗口，资源占用更高，也更容易触发风控（建议先从 2 开始试）
7.（可选）设置「并行抖动（秒）」：默认 4 秒；并发时每个账号会随机延迟 0~抖动秒数再启动，避免多个账号“同步操作”
