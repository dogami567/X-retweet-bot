...
3. **准备图片**：
   ...
   - 运行中也可以随时继续往目录放图/上传；为避免“拷贝未完成的半成品图片”被选中，新图片会延迟约 2~3 秒才出现在图库/参与随机选图
4. **添加账号并登录**：
   - 左侧「账号管理」点 `+` 新建账号
...
## 当前状态与已知限制（2026-03-11）

> 这一节用于同步当前主链路已经落地的能力、仍需人工前置的限制，以及发布前推荐的回归入口。

### 当前已落地能力

1. **关键词采集与 URL 队列已形成可配置闭环**
   - 采集策略支持“每天一次”与“队列一轮后补采（queue_cycle）”。
   - 支持配置 `harvestMinIntervalSec`，避免队列轮转时重复触发采集。
   - Bulk 面板与状态接口会展示 `lastTrigger`、`lastRequestedAt`、`lastQueueCycleAt`、当前 URL 指针与等待原因。

2. **关注评论用户过滤规则已补齐**
   - 支持“仅关注已认证账号（蓝 V）”。
   - 支持“仅关注 bio 含中文账号”。
   - 过滤发生在真正点击关注前；如果候选资料读取失败/未知，会按“保守放行”处理，避免误杀。

3. **批量发帖已支持运行中热更新内容池**
   - 运行中的 bulk scheduler 在保存 captions / 图片目录等内容池配置时，不再全局 stop/start。
   - 图库 watcher 会自动发现新图；新图在“稳定窗口”内不会被选中，避免半成品文件误发。
   - 当文案和图片都不可用时，任务会记为 `skip`，不会因为空内容直接崩溃。

4. **核心自动化回归已补齐本地稳定入口**
   - 主回归：`npm run test:bulk-regression`
   - Bulk UI / 热更新 e2e：`npm run test:e2e:bulk-core`
   - 真实采集验收：`npm run test:e2e:harvest-acceptance`

### 当前仍需注意的限制

- `harvest` / `follow-commenters` 的真实链路仍依赖本机浏览器 Profile 登录态、代理/网络环境，以及 X 页面结构是否变化。
- 蓝 V / 中文 bio 过滤依赖页面资料读取；若 X 页面临时缺字段，会按“未知即放行”处理，而不是直接过滤。
- `npm run test:e2e:harvest-acceptance` 属于真实环境验收，不适合在没有登录态/可访问 x.com 的环境中当作稳定 CI 信号。

### 现阶段使用建议

- 如果想长期跑“采集 -> 自动关注”，推荐把采集策略切到“队列一轮后补采”，同时把最小间隔设到 15 分钟以上，避免频繁补采。
- 如果对关注质量有要求，保存 bulk 配置前先勾选“仅关注已认证账号 / 仅关注 bio 含中文账号”，再点“开始”。
- 如果需要运行中补图或补文案，直接保存即可；保存后无需手动 stop/start bulk scheduler。
- 每次准备打包或交付给客户前，至少先跑一遍 `npm run test:bulk-regression`；真实采集链路改动后，再补一遍 `npm run test:e2e:harvest-acceptance`。
