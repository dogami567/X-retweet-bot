...
## 当前问题与开发进展（2026-03-10）

> 这一节用于同步当前代码现状、已确认问题和接下来的开发方向，方便线上用户与维护者快速对齐预期。

### 当前已确认的问题

1. **关键词采集与 URL 队列还没有形成完整闭环**
   - 目前支持“手动采集一次”或“每天自动采集一次”。
   - 现在**还不支持**“当前 URL 队列全部跑完后，自动重新采集一轮关键词，再继续跑”。
   - 这意味着想长期连续跑时，现阶段仍可能需要人工补队列，或依赖每天自动采集一次。

2. **关注评论用户的筛选规则还不完整**
   - 当前关注流程已经支持：跳过已关注、已请求、受保护账号，检测登录失效与限流/风控。
   - 但目前**还没有正式实现**以下筛选：
     - 没有蓝 V 的账号不关注
     - 简介 / Bio 全英文、没有中文信号的账号不关注

3. **批量 24 小时运行能力已具备基础，但“热更新”还不够彻底**
   - 当前批量调度器本身是持续运行模型，不是跑一轮就退出。
   - 图库目录支持运行中新增图片，后续任务可以继续拾取。
   - 但如果修改 bulk 配置（例如文案、账号参数等），当前仍会触发一次“停止后再启动”的软重启，不是完全无感热更新。

4. **长链路自动化测试还不够覆盖真实生产场景**
   - 当前已有基础 UI / 队列 / 一次性采集 / 无账号启动失败等测试。
   - 但以下场景仍缺少自动化回归：
     - 队列跑完后自动重新采集
     - 蓝 V / 中文简介过滤
     - 运行中新增文案 / 图片后继续调度
...
- 已完成针对以下模块的专项代码排查：
  - 关键词采集（harvest）
  - URL 队列与关注评论（follow-commenters）
  - 批量发帖调度（bulk scheduler）
  - 图库扫描与运行中补图（image watcher）
- 已确认当前问题里，**有一部分不是“小 bug”，而是机制尚未补齐**，后续会按优先级补上。
- 已梳理出下一阶段开发顺序：
  1. 先补“队列跑完自动重采集”闭环
  2. 再补“蓝 V / 中文简介过滤”
  3. 再补“批量运行中的热更新与不中断”
  4. 最后补齐回归测试并逐步拆分大文件逻辑

### 现阶段使用建议

- 如果你现在主要依赖“关键词采集 + 自动关注”，请先按“每日自动采集”理解当前行为，而不要默认它会在队列跑完后立刻重采集。
- 如果你对关注对象有较强质量要求（例如必须蓝 V、必须带中文简介），当前版本还不能完全满足，需要等待后续规则补齐。
- 如果你在批量运行中需要补图，当前已支持；如果你要改大段文案或批量配置，建议在低峰时段保存，避免碰到调度软重启带来的短暂抖动。

### 后续更新方式

- 本节会随着修复进度持续更新。
- 每次解决一类核心问题后，会同步补充到 `CHANGELOG.md`。
